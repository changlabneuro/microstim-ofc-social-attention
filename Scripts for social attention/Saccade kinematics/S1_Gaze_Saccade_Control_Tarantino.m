...
% Upload Gaze_Data_Tarantino.mat and Saccade_Tarantino.mat
        ...
        for nstim=1:length(stim_ts)        
            ...
            if GazeData.(field_id).(run_id).stim_ts(nstim) >= min(GazeData.(field_id).(run_id).t) && GazeData.(field_id).(run_id).stim_ts(nstim) < max(GazeData.(field_id).(run_id).t)-5
                ...
                if isempty(macrosaccade_idx)
                    ...
                    % Label contra- or ipsi- lateral for each event (Tarantino's chamber is on the right side of the head)
...
save('SaccadeGaze_Data_Tarantino.mat','GazeSaccade','-v7.3');  
